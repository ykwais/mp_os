    ...
    bool upper_bound_inclusive)
{
    ...
    btree_node* node_end = *(stk.top().first);
    ...
    {
        auto [in_ind, found] = find_index(lower_bound, node);
        if (in_ind > node->_virtual_size - 1)
        {
            in_ind = node->_virtual_size - 1;
        }

        end.update_index(in_ind);
    }
